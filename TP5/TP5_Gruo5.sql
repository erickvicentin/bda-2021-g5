#1_a
create user 'jPerez'@'%' identified by '1608';
create user 'jPerez'@'localhost' identified by '1608';
create user 'aFernandez'@'%' identified by '1608';
create user 'aFernandez'@'localhost' identified by '1608';
...
# Le tengo que dar permisos de Admin al usuario wizard, para grant
...
GRANT SELECT (address_id,address,address2, district, city_id, postal_code,phone ,location) 
ON sakila.address TO 'jPerez'@'%';

CREATE VIEW sin_last_update AS 
...
GRANT CREATE VIEW ON sakila.* TO 'jPerez'@'%';

revoke select on sakila.address.last_update from 'jPerez'@'%';
