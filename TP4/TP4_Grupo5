...
VALUES  ('Pedro Lopez','Las Heras 521','379154252525'),
        ...
        ('Sofia Lucero','Las Montañas 422','0379154959595');
...
create procedure mostrar_estadisticas_prueba (num int)
    begin
        case num
            ...
            when 2 then
                -- Valor total de las ventas en el año 2011.
            ...
            when 4 then
                ...
                select id_Venta as Venta, (unidades_vendidas*precio_unitario) as Precio_Total, dayname(fecha) as Día
                from ventas
                order by dayofweek(fecha) asc;
            when 5 then
                -- Listado ordenado de las ventas por mes del año.
                select id_Venta as Venta, (unidades_vendidas*precio_unitario) as Precio_Total, monthname(fecha) Mes, year(fecha) as Año
                from ventas
                order by month(fecha) asc;
            when 6 then
                -- Listado ordenado de la ventas por cuatrimestre del año.
                select id_Venta as Venta, (unidades_vendidas*precio_unitario) as Precio_Total, quarter(fecha) as Cuatrimestre, year(fecha) as Año
                from ventas
